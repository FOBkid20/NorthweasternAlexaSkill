...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speak_output = "Congratulations on your new position! Do you know your start date yet?"
        reprompt_text = "I started at Amazon on Nov. 6th, 2014. When are you starting?"
        return (
...
class HasStartDateLaunchRequestHandler(AbstractRequestHandler):
    """Handler for launch after they have set their start date"""
    ...
    def handle(self, handler_input):
        ...
        # getting the next start date
        month_as_index = list(calendar.month_abbr).index(month[:3].title())
        next_startdate = datetime(current_year, month_as_index, day)

        # check if we need to adjust bday by one year
        if now_date > next_startdate:
            ...
            current_year += 1
        # setting the default speak_output to Happy xth start date!!
        # Alexa will automatically correct the ordinal for you.
        # no need to worry about when to use st, th, rd
        speak_output = "Happy {}th start date!".format(str(current_year - year))
        if now_date != next_startdate:
            diff_days = abs((now_date - next_startdate).days)
            speak_output = "Welcome back. It looks like there are \
                            {days} days until your {startdate_num}th\
                            startdate".format(
                                days=diff_days,
                                startdate_num=(current_year-year)
                            )
        handler_input.response_builder.speak(speak_output)

        return handler_input.response_builder.response

class CaptureStartDateIntentHandler(AbstractRequestHandler):
    """Handler for Hello World Intent."""
    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
        return ask_utils.is_intent_name("CaptureStartDateIntent")(handler_input)
    ...
    def handle(self, handler_input):
        ...
        startdate_attributes = {
            "year": year,
            "month": month,
            "day": day
        }

        attributes_manager.persistent_attributes = startdate_attributes
...
sb.add_request_handler(HasStartDateLaunchRequestHandler())
sb.add_request_handler(LaunchRequestHandler())
sb.add_request_handler(CaptureStartDateIntentHandler())
